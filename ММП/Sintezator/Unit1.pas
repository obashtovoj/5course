...
var
  ...
  hMidi,t,no,midimsg,n,n2:integer; // no-нота, midimsg-мидисмс
...
{$R *.dfm}
//підключити інструменти із звукової бази операційної системи, тобто
//заповнити midi-пристрої.


procedure TimeWrite(T : DWORD);                                                 // запись дельта-времени в виде величины переменной длинны
var
...
Timer1.Enabled := True;
midimsg := $C0+$100*65;  //говоримо, яким інструментом його зіграти
...
midimsg := $90 + ((no) * $100) + ((vel) * $10000) + 0; // выибраем аккорд
midiOutShortMsg (hmidi, midimsg);
if Timer2.Enabled then                                                      // если включена запись
          begin
            TimeWrite(Tim);
...
midiOutShortMsg(hmidi, MidiMSG);
 if Timer2.Enabled then                                                      // если включена запись
   ...
   //Процедура натиснення функціональної клавіші  мишки
